...
const $cancelModal = document.querySelector('.cancel-modal') as HTMLElement;
if ($cancelModal) throw new Error('$cancelModal does not exist');
...
const $confirmModal = document.querySelector('.confirm-modal') as HTMLElement;
if ($confirmModal) throw new Error('$confirmModal does not exist');
...
function deleteEntries(entryId: number): void {
  const entriesArray = data.entries.filter(
